...
<script>
    ...
    window.dataLayer.push({
        ...
        "ecommerce": {
            "currency": "BDT",
            "value": {{ $product->discount_price > 0 ? $product->discount_price : $product->regular_price }},
            "items": [
                {
                    "item_id": "{{ $product->id }}",
...
@push('css')
    ...
    <style>
        ...
        /* Custom Image Slider CSS - SIMPLE AND RELIABLE */
        ...
        /* Fix for color selection images */
        .color img {
            height: 70px;
            width: 70px;
            object-fit: contain;
...
@section('content')
    ...
    <div class="banner-bootom-w3-agileits">
        <div class="container">
            <div class="row" style="background: white;margin-top: 20px;padding: 20px 10px;box-shadow: 0px 4px 6px -3px #9c9c9c;">

                <div class="col-md-4 single-right-left">
                    <div class="grid images_3_of_2">
                        <div class="product-slider-container" style="padding: 12px; box-shadow: none; border: 1px solid #e0e0e0;">
                            <!-- Main Image Slider -->
                            <div class="main-slider" id="mainSlider">
                                    ...
                                    <div class="main-slide active" data-index="0">
                                        <div class="video-wrap">
                                            <div class="video-embed">
                                                <iframe src="{{ $product->yvideo }}" frameborder="0" allowfullscreen></iframe>
                                            </div>
                                        </div>
                                    </div>
                                    @php $video_index = 0; @endphp
                                ...
                                @foreach ($product->images as $index => $image)
                                    <div class="main-slide {{ !isset($video_index) && $loop->first ? 'active' : '' }}" 
                                         data-index="{{ isset($video_index) ? $index + 1 : $index }}"
                                         data-cl="{{ $image->color_attri }}">
                                        <div class="thumb-image">
                                            <a href="{{ asset('uploads/product/' . $image->name) }}" target="_blank">
                                                <img src="{{ asset('uploads/product/' . $image->name) }}" class="my-gallery-image img-responsive" alt="">
                                            </a>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            <div class="thumbnail-container" id="thumbnailContainer">
                                @if ($product->yvideo || $product->video)
                                    <div class="thumbnail-item active" data-index="0">
                                        <img src="{{ asset('uploads/product/video/' . $product->video_thumb) }}" alt="Video">
                                    </div>
                                @endif
                                
                                @foreach ($product->images as $index => $image)
                                    <div class="thumbnail-item {{ !isset($video_index) && $loop->first ? 'active' : '' }}" 
                                         data-index="{{ isset($video_index) ? $index + 1 : $index }}"
                                         data-cl="{{ $image->color_attri }}">
                ...
                <div class="col-md-5 single-right-left simpleCart_shelfItem">
                    ...
                    @if ($product->sku)
                        <p>sku - <b><i>{{ $product->sku }}</i></b></p>
                    @endif
                    @isset($product->user->shop_info->name)

                    @endisset
                    @isset($product->brand->name)
                        <p>Brand: <a style="font-size:14px; color: #369336;"
                    ...
                    <p>
                        ...
                        @if ($product->whole_price > 0)
                            <p class="item_price">Whole Sell:
                                {{ setting('CURRENCY_CODE_MIN') ?? 'TK' }}.{{ $product->whole_price }}</p>
                        @endif
                    </p>

                    <p style="font-size: 13px">
                    <p style="margin:0;font-size: 11px;margin-left: 21px;margin-top: 4px;">
                        @if (setting('COUNTRY_SERVE') == 'Bangladesh')
                        ...
                        <div class="row ml-1">
                            ...
                            <div class="btn-group btn-group-toggle btn-color-group d-block mt-n2 ml-n2"
                                ...
                                @foreach ($colors_product as $color)
                                    <label
                                        class="btn p-3 m-1 color {{ $product->colors->count() == 1 ? 'active' : '' }}"
                    ...
                    @foreach ($attributes as $attribute)
                        @php
                            $values = $product->attributes_values->where('attributes.id', $attribute->id);
                        @endphp
                            ...
                            <div class="row ml-1 mb-2">
                                ...
                                @foreach ($values as $value)
                                    <div class="inline-block">
                                        <input type="radio" name="{{ $attribute->slug }}"
                                            id="attr_{{ $attribute->id }}_{{ $value->id }}"
                                            value="{{ $value->id }}" class="form-check-input get_attri_price"
                        ...
                        <div class="snipcart-details top_brand_home_details item_add single-item hvr-outline-out">
                            <form action="{{ route('add.cart') }}" method="POST" id="addToCart">
                                ...
                                <fieldset>
                                    ...
                                    <input type="hidden" name="color" value="blank">

                                    {{-- Attribute Hidden Inputs --}}
                                    @foreach ($attributes as $attribute)
                                        @php
                                            $values = $attributeGroups->get($attribute->id, collect());
                                        @endphp

                                        @if ($values->isNotEmpty())
                                            <input type="hidden" name="{{ $attribute->slug }}"
                                                id="{{ $attribute->slug }}"
                                                value="{{ $values->count() === 1 ? $values->first()->id : 'blank' }}">
                                        @endif
                                    @endforeach

                                    {{-- Add to Cart Button --}}
                                    @if ($product->quantity > 0 && $product->sheba != 1)
                                        <button type="submit" name="submit" class="redirect"
                                            style="width:140px;margin-top: 10px;background:#ff9900;color:white">
                            ...
                            <div class="snipcart-details top_brand_home_details item_add single-item hvr-outline-out">
                                <form action="{{ route('buy.product') }}" method="GET">
                                    ...
                                    <fieldset>
                                        ...
                                        @else
                                            <input
                                                style="width:140px;margin-top: 10px;background: var(--primary_color);color: white;border-color: var(--primary_color); margin-left:10px;"
                ...
                <div style="margin-top: 10px;font-size: 12px;">
                    <div class="dropdown row single-product-button" style="padding: 0;margin: 0;">
                        ...
                        @if (App\Models\wishlist::where('user_id', auth()->id())->where('product_id', $product->id)->first())
                            <?php
                            $k = App\Models\wishlist::where('user_id', auth()->id())
                        ...
                        @else
                            <form style="padding:0;margin-top: 12px;"
                                class="col-md-3 col-4"action="{{ route('wishlist.add') }}" method="post">
                                @csrf
            ...
            <div class="col-md-3 new_r">
                ...
                @foreach ($relatedProducts as $relatedProduct)
                    <x-single-related :product="$relatedProduct" classes="" />
                @endforeach
            </div>
        </div>
        <br>
                ...
                <div id="reviews" class="collapse" aria-labelledby="headingThree" data-parent="#accordion">
                    <div class="card-body">
                        <div class="review">
                            ...
                            <div class="{{ $product->reviews->count() > 0 ? 'card' : '' }}"
        ...
        <div class="products related row"
            ...
            <div class="container">
                ...
                <div class="row autoplay">
                    @forelse ($categoryBestProducts as $catProduct)
                        <x-product-grid-view :product="$catProduct" classes="" />
                    @empty
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            // SIMPLE CUSTOM SLIDER - 100% WORKING
            let currentSlide = 0;
            ...
            function showSlide(index) {
                ...
                // Reinitialize image zoom for the active slide
                setTimeout(() => {
                    const activeImage = document.querySelector('.main-slide.active .my-gallery-image');
                    if (activeImage && window.matchMedia('(min-width: 768px)').matches) {
            ...
            // Color change event - filter slides by color
            $(document).on('click', '.color', function(e) {
                ...
                if (value !== 'blank') {
                    // Show only slides with matching color
                    let foundVisibleSlide = false;
                    let firstVisibleIndex = 0;
                    
                    mainSlides.forEach((slide, index) => {
                        const colorAttr = slide.getAttribute('data-cl');
                        if (colorAttr === value || slide.querySelector('.video-wrap')) {
                            // This slide should be visible
                            if (!foundVisibleSlide) {
                                firstVisibleIndex = index;
                                foundVisibleSlide = true;
                            }
                        }
                    });
                    
                    // Show the first matching slide
                    if (foundVisibleSlide) {
                        showSlide(firstVisibleIndex);
                    }
                } else {
                    // Show all slides - go to first slide
                    showSlide(0);
                }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (typeof loadCartOnCanvas === "function") {
                loadCartOnCanvas();
            }

           
            let alertType = response.alert || 'Success';
            let alertMsg = response.message || 'Product added successfully!';

            let messageHtml = '<div class="col-12 mt-2">';
            if (alertType.toLowerCase() === 'warning' || alertType.toLowerCase() === 'error') {
            ...
            messageHtml += '<strong>' + alertType + '! </strong> ' + alertMsg;
            messageHtml += '</div></div>';

           
            $('#response-message').html(messageHtml);
        ...
        error: function(xhr) {
            ...
            let messageHtml = '<div class="col-12 mt-2">';
            messageHtml += '<div class="alert alert-danger alert-dismissible fade show" role="alert">';
            messageHtml += '<strong>Oops!! </strong>' + errorMsg;
            messageHtml += '</div></div>';
            ...
            // Initialize image zoom for desktop on page load
            setTimeout(() => {
