...
public function all_transation_reports()
  {
  $data = ['title' => 'Transation Report'];
  ...
  if(isset($_GET['search']))
    ...
    if($report == 'dailyReports')
      ...
      $data['pruchase'] = $this->pm->get_dpurchase_data($sdate,$edate,$ttype,$acid);
      $data['sale'] = $this->pm->get_dsale_data($sdate,$edate,$ttype,$acid);
      $data['sreturn'] = $this->pm->get_dsreturn_data($sdate,$edate,$ttype,$acid);
      $data['preturn'] = $this->pm->get_dpreturn_data($sdate,$edate,$ttype,$acid);
      $data['voucher'] = $this->pm->get_dvoucher_data($sdate,$edate,$ttype,$acid);
      }
    else if ($report == 'monthlyReports')
      {
      $month = $_GET['month'];
      ...
      $data['pruchase'] = $this->pm->get_mpurchase_data($month,$year,$ttype,$acid);
      $data['sale'] = $this->pm->get_msale_data($month,$year,$ttype,$acid);
      $data['sreturn'] = $this->pm->get_msreturn_data($month,$year,$ttype,$acid);
      $data['preturn'] = $this->pm->get_mpreturn_data($month,$year,$ttype,$acid);
      $data['voucher'] = $this->pm->get_mvoucher_data($month,$year,$ttype,$acid);
      }
    else if ($report == 'yearlyReports')
      {
      $year = $_GET['ryear'];
      ...
      $data['pruchase'] = $this->pm->get_ypurchase_data($year,$ttype,$acid);
      $data['sale'] = $this->pm->get_ysale_data($year,$ttype,$acid);
      $data['sreturn'] = $this->pm->get_ysreturn_data($year,$ttype,$acid);
      $data['preturn'] = $this->pm->get_ypreturn_data($year,$ttype,$acid);
      $data['voucher'] = $this->pm->get_yvoucher_data($year,$ttype,$acid);
      }
    }
  else
    {
    $data['pruchase'] = $this->pm->get_purchses_data();
    $data['sale'] = $this->pm->get_sales_data();
    $data['sreturn'] = $this->pm->get_return_bs_data();
    $data['preturn'] = $this->pm->get_preturn_data();
    $data['voucher'] = $this->pm->get_voucher_data();
    }

  $this->load->view('bankaccount/transation_report',$data);
}
